...
line1, = ax.plot(x, conc_0_history[0], label='željeni protein', color=palette[0])
line2, = ax.plot(x, conc_1_history[0], label='nečistoča', color=palette[1])
line3, = ax.plot(x, conc_2_history[0], label='nečistoča', color=palette[2])
ax.set_xlim(0, 1)
ax.set_ylim(0, 1.2)
ax.set_xlabel("Pozicija v kromatografu")
ax.set_ylabel("Koncentracija")
ax.legend()
ax.set_title("Separacija tekočih faz")

# Show the animation
plt.show()
# Store the profiles for video generation
'''
# Select four specific time frames for the images
time_frames = [1*400-1, 4*500-1,8*500-1,16*500-1]
time_labels = [f"t = {frame * dt:.2f}" for frame in time_frames]

# Create a 2x2 subplot
fig, axes = plt.subplots(2, 2, figsize=(12, 6))
axes = axes.flatten()

# Plot the liquid-phase concentrations for the selected time frames
for i, frame in enumerate(time_frames):
    ax = axes[i]
    ax.plot(x, conc_0_history[frame], label='željeni protein', color=palette[0])
    ax.plot(x, conc_1_history[frame], label='nečistoča1', color=palette[2])
    ax.plot(x, conc_2_history[frame], label='nečistoča2', color=palette[8])
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1.2)
    ax.set_xlabel("Pozicija v kromatografu")
    ax.set_ylabel("Koncentracija")
...
plt.tight_layout()
plt.savefig("protein_separation_2x2_kot_david2.png", dpi=300)  # Save the figure as a PNG
plt.show()
