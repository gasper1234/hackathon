    ...
    while t < t_max:
        ...
        for i in range(N_species):
            time_der = time_derivative(
                conc[i, 0, 0, :], conc[i, 0, 1, :], conc[i, 0, 2, :], conc[i, 1, 0, :],
            (u, D[i], alpha[i])
            )
            ...
            sum_term = (np.sum(conc[i, 0, 0, 1:]) * dx / (1 - delez_smole)
        ...
        conc_0_history.append(conc[0, 0, 0, :].copy())
        conc_1_history.append(conc[1, 0, 0, :].copy())
        conc_2_history.append(conc[2, 0, 0, :].copy())
        #print(conc[0, 2, 0, :])
        conc_0_v_history.append(conc[0, 1, 0, :].copy())
        conc_1_v_history.append(conc[1, 1, 0, :].copy())
        t += dt
...
k_q = 100
B_q = 0.00001 # Disociacijska konstanta za kislino
A_q = 10.0
...
conc[2, 0, 0, 0] = zac_conc[2]  # Protein B
...
t_max = 20.0
required_purity = 0.7
u_array = np.linspace(0.1, 0.1, int(t_max)) / (1 - delez_smole)
arr1 = np.tile(np.array([[0.2, 0.1]]), (int(t_max), 1))
...
gs = GridSpec(1, 2, width_ratios=[2, 1])  # Two subplots: left (2x width), right (1x width)

# Left plot: Liquid-phase concentrations
ax1 = fig.add_subplot(gs[0])
line1, = ax1.plot(x, conc_0_history[0], label='željeni protein', color=palette[0])
line2, = ax1.plot(x, conc_1_history[0], label='nečistoča1', color=palette[2])
line3, = ax1.plot(x, conc_2_history[0], label='nečistoča2', color=palette[7])
ax1.set_xlim(0, 1)
ax1.set_ylim(0, 1.2)
ax1.set_xlabel("Pozicija v kromatografu")
ax1.set_ylabel("Koncentracija")
ax1.legend()
ax1.set_title("Separacija tekočih faz")

# Right plot: Purity in the chromatograph
ax2 = fig.add_subplot(gs[1])
purity_line, = ax2.plot(x, np.zeros_like(x), label="Čistost", color=palette[3])
ax2.set_xlim(0, 1)
ax2.set_ylim(0, 1.2)
ax2.set_xlabel("Pozicija v kromatografu")
ax2.set_ylabel("Čistost")
ax2.legend()
ax2.set_title("Čistost v kromatografu")

# Update function for animation
def update(frame):
    print(f"Updating frame: {frame}, Time: {frame * dt:.2f}")
    # Update liquid-phase concentrations
    line1.set_ydata(conc_0_history[frame])
    line2.set_ydata(conc_1_history[frame])
    line3.set_ydata(conc_2_history[frame])
    
    # Calculate purity at each spatial position
    purity = conc_0_history[frame] / (conc_0_history[frame] + conc_1_history[frame] +  conc_2_history[frame] + 1e-12)
    purity_line.set_ydata(purity)
    
    # Update titles with time
    ax1.set_title(f"Separacija tekočih faz")
    ax2.set_title(f"Čistost v kromatografu")
    return line1, line2, line3, purity_line

# Create the animation
ani = FuncAnimation(fig, update, frames=range(0,len(conc_0_history),140), blit=True)

# Save the animation as a GIF
ani.save("protein_separation_with_chromatograph_purity_kot_david.gif", writer="imagemagick", fps=60)

# Show the animation
plt.show()
# Store the profiles for video generation
