...
if __name__ == "__main__":
    # Compute yield values over time
    koncno = compute_outflows(
        evolution_data["all_outflows"], zac_conc, dt, t_max, required_purity, N_species
    )
    yield_values_time = [koncno[i][0] for i in range(len(koncno))]

    # Compute yield values for purity
    purity = [0.005 * i for i in range(1, 201)]
    yield_values_purity = []
    for p in purity:
        koncno = compute_outflows(
            evolution_data["all_outflows"], zac_conc, dt, t_max, p, N_species
        )
        yield_values_purity.append(koncno[-1, 0])

    # Create two separate subplots
    fig, axes = plt.subplots(1, 2, figsize=(10, 6))

    # First subplot: Yield over time
    ax1 = axes[0]
    ax1.plot(np.linspace(0, 80, len(yield_values_time)), yield_values_time, color=palette[0], label="Maksimalni izkoristek")
    ax1.set_xlabel("Čas (a.u.)")
    ax1.set_ylabel("Maksimalni izkoristek pri čistosti 70%", color=palette[0])
    ax1.tick_params(axis="y", labelcolor=palette[0])
    #ax1.set_title("Maksimalni izkoristek skozi čas")
    ax1.legend(loc="upper left")

    # Second subplot: Yield vs. purity
    ax2 = axes[1]
    ax2.plot(purity, yield_values_purity, color=palette[1], label="Izkoristek")
    ax2.set_xlabel("Čistost")
    ax2.set_ylabel("Izkoristek", color=palette[1])
    ax2.tick_params(axis="y", labelcolor=palette[1])
    #ax2.set_title("Izkoristek pri določeni čistosti")
    ax2.legend(loc="upper right")

    # Adjust layout and show the plots
    plt.tight_layout()
    plt.savefig("yield_vs_purity_and_time_kot_david.png", dpi=300)  # Save the figure as a PNG
    plt.show()
