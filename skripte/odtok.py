...
arr1 = np.tile(np.array([[0.2, 0.2]]), (int(t_max), 1))
arr2 = np.tile(np.array([[0.4, 0.4]]), (int(t_max) // 2, 1))
zac_soli_array = arr1
...
if __name__ == "__main__":
    # Extract outflows for both species
    outflow_0 = evolution_data["all_outflows"][0]
    outflow_1 = evolution_data["all_outflows"][1]
    outflow_2 = evolution_data["all_outflows"][2]

    # Compute derivatives (rate of change) of the outflows
    derivative_outflow_0 = np.gradient(outflow_0, dt)
    derivative_outflow_1 = np.gradient(outflow_1, dt)
    derivative_outflow_2 = np.gradient(outflow_2, dt)

    # Create a subplot with two plots in a row
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))

    # Left plot: Outflows
    ax1 = axes[0]
    ax1.plot(np.linspace(0, t_max, len(outflow_0)), outflow_0, color=palette[0], label="željeni protein")
    ax1.plot(np.linspace(0, t_max, len(outflow_1)), outflow_1, color=palette[2], label="nečistoča1")
    ax1.plot(np.linspace(0, t_max, len(outflow_2)), outflow_2, color=palette[7], label="nečistoča2")
    ax1.set_xlabel("Čas (a.u.)")
    ax1.set_ylabel("Odtok (a.u.)")
    #ax1.set_title("Odtoki za željeni protein in nečistočo")
    ax1.legend()

    # Right plot: Derivatives of outflows
    ax2 = axes[1]
    ax2.plot(np.linspace(0, t_max, len(derivative_outflow_0)), derivative_outflow_0, color=palette[0], label="željeni protein")
    ax2.plot(np.linspace(0, t_max, len(derivative_outflow_1)), derivative_outflow_1, color=palette[2], label="nečistoča1")
    ax2.plot(np.linspace(0, t_max, len(derivative_outflow_2)), derivative_outflow_2, color=palette[7], label="nečistoča2")
    ax2.set_xlabel("Čas (a.u.)")
    ax2.set_ylabel("Sprememba odtoka (a.u.)")
    #ax2.set_title("Sprememba odtokov skozi čas")
    ax2.legend()

    # Adjust layout and show the plots
    plt.tight_layout()
    plt.savefig("outflows_and_derivatives_plot.png", dpi=300)  # Save the figure as a PNG
    plt.show()
